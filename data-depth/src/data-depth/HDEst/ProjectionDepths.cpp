...
xp{ new double[n] {} }
{
	...
	Method[11] = [this](const double* z) { return CoordinateDescentGCMultiStart(z); };
	Method[12] = [this](const double* z) { return NelderMeadGCMultiStart(z); };
...
double cProjection::Depth(const double* z) {
	...
	double depth = Method[(int)_Method](z); // depth calculation
...
double cProjection::RefinedRandom(const double* z) {
	...
	do {
		...
		for (int i = 0; i < _nRandom / _maxRefinesRand; i++) {
...
double cProjection::NelderMead(const double* z){
	...
	for (int i = 0; i < d + 1; i++) fevals[i].arg = new double[d];
