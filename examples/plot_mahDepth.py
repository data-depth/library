...
Sample usage of Mahalanobis depth computation.
...
depthX=model.mahalanobis(X,exact=True) # Compute X Depth w.r.t. the dataset  
depthDataset=model.mahalanobis(exact=True, evaluate_dataset=True) # evaluate the dataset itself
...
for x, depth, name, ax in zip([dataset,X],[depthDataset,depthX],
    ...
    ax.set_title(f"{name} mahalanobis depth visualization")
ax2.legend()
plt.show()
