...
Sample usage of simplicial depth computation.
It will plot samples and dataset based on simplicial depth values.
...
depthX=model.simplicial(X,exact=True) # Compute X Depth w.r.t. the dataset  
depthDataset=model.simplicial(exact=True, evaluate_dataset=True) # evaluate the dataset itself
...
for x, depth, name, ax in zip([dataset,X],[depthDataset,depthX],
    ...
    ax.set_title(f"{name} simplicial depth visualization")
ax2.legend()
plt.show()
