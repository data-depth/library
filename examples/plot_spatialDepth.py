...
Sample usage of spatial depth computation.
It will plot samples and dataset based on spatial depth values.
...
depthX=model.spatial(X) # Compute X Depth w.r.t. the dataset  
depthDataset=model.spatial(evaluate_dataset=True) # evaluate the dataset itself
...
for x, depth, name, ax in zip([dataset,X],[depthDataset,depthX],
    ...
    ax.set_title(f"{name} spatial depth visualization")
ax2.legend()
plt.show()
