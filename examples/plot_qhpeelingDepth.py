...
Sample usage of convex hull peeling depth computation.
It will plot samples and dataset based on convex hull peeling depth values.
...
depthX=model.qhpeeling(X) # Compute X Depth w.r.t. the dataset  
depthDataset=model.qhpeeling(evaluate_dataset=True) # evaluate the dataset itself
...
for x, depth, name, ax in zip([dataset,X],[depthDataset,depthX],
    ...
    ax.set_title(f"{name} convex hull peeling depth visualization")
ax2.legend()
plt.show()
