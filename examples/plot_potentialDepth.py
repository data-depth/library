...
Sample usage of potential depth computation.
It will plot samples and dataset based on potential depth values.
...
depthX=model.potential(X) # Compute X Depth w.r.t. the dataset  
depthDataset=model.potential(evaluate_dataset=True) # evaluate the dataset itself
...
for x, depth, name, ax in zip([dataset,X],[depthDataset,depthX],
    ...
    ax.set_title(f"{name} potential depth visualization")
ax2.legend()
plt.show()
