...
Sample usage of beta skeleton depth computation.
It will plot samples and dataset based on beta skeleton depth values.
...
depthX=model.betaSkeleton(X,) # Compute X Depth w.r.t. the dataset  
depthDataset=model.betaSkeleton(evaluate_dataset=True,) # evaluate the dataset itself
...
for x, depth, name, ax in zip([dataset,X],[depthDataset,depthX],
    ...
    ax.set_title(f"{name} beta skeleton depth visualization")
ax2.legend()
plt.show()
