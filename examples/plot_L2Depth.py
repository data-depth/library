...
Sample usage of L2 depth computation.
It will plot samples and dataset based on L2 depth values.
...
depthX=model.L2(X) # Compute X Depth w.r.t. the dataset  
depthDataset=model.L2(evaluate_dataset=True) # evaluate the dataset itself
...
for x, depth, name, ax in zip([dataset,X],[depthDataset,depthX],
    ...
    ax.set_title(f"{name} L2 depth visualization")
ax2.legend()
plt.show()
