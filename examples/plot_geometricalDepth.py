...
Sample usage of Geometrical depth computation.
It will plot samples and dataset based on geometrical depth values.
...
depthX=model.geometrical(X,output_option="lowest_depth") # Compute X Depth w.r.t. the dataset  
depthDataset=model.geometrical(evaluate_dataset=True, output_option="lowest_depth") # evaluate the dataset itself
...
for x, depth, name, ax in zip([dataset,X],[depthDataset,depthX],
    ...
    ax.set_title(f"{name} geometrical depth visualization")
ax2.legend()
plt.show()
